...
function StatusPill({ status }: Readonly<{ status: string }>) {
  const s = STATUS_MAP[status] ?? { label:status, bg:'#F8FAFC', dot:'#94A3B8', text:'#475569' };
  return (
...
function QuickBtn({ label, icon, href, primary, onNav }: QuickBtnProps) {
  ...
  return (
    <button type="button" onClick={handleClick} style={{
      ...
      background: primary ? 'linear-gradient(135deg,#1D4ED8,#3B82F6)' : '#fff',
      color: primary ? '#fff' : '#334155',
      fontSize:'13.5px', fontWeight:600, cursor:'pointer', fontFamily:'inherit',
      boxShadow: primary ? '0 4px 14px rgba(29,78,216,0.3)' : '0 1px 4px rgba(0,0,0,0.06)',
...
export default function DashboardPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (invRes) {
        ...
        else if (Array.isArray(r?.invoices)) list = r.invoices;
        setInvoices(list);
      }
  ...
  const usedPct = planData && !planData.isUnlimited && planData.invoicesLimit > 0
    ? Math.min(100, Math.round((planData.invoicesUsed / planData.invoicesLimit) * 100))
    : 0;
  const quotaWarning = planData && !planData.isUnlimited && usedPct >= 80;
    ...
    <div style={{ padding:'32px 36px 48px', background:'#F1F5F9', minHeight:'100vh', fontFamily:'system-ui,-apple-system,sans-serif' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:'16px', marginBottom:'24px' }}>
        <StatCard label="Total facturas" value={invoices.length} loading={loading} color="#6366F1"
          sub={planData ? `${planData.invoicesUsed}/${planData.isUnlimited?'∞':planData.invoicesLimit} usadas este mes` : undefined}
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:'20px', alignItems:'start' }}>
        ...
        <div style={{ background:'#fff', borderRadius:'18px', border:'1px solid #F1F5F9', boxShadow:'0 1px 4px rgba(0,0,0,0.04)', overflow:'hidden' }}>
          ...
          {loading ? (
            <div style={{ padding:'40px', textAlign:'center', color:'#94A3B8', fontSize:'13px' }}>Cargando...</div>
          ) : recent.length === 0 ? (
            ...
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              ...
              <tbody>
                {recent.map(inv => <InvRow key={inv.id} inv={inv} onNav={handleInvNav} />)}
              </tbody>
            </table>
          )}
        </div>
            ...
            {planData ? (
              <div>
                ...
                <div style={{ height:'6px', background:'rgba(255,255,255,0.1)', borderRadius:'99px', overflow:'hidden', marginBottom:'6px' }}>
                  <div style={{ height:'100%', width:`${planData.isUnlimited ? 100 : usedPct}%`, background: usedPct >= 90 ? '#EF4444' : 'linear-gradient(90deg,#6366F1,#818CF8)', borderRadius:'99px', transition:'width 0.6s' }} />
                </div>
