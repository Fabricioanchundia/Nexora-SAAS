...
function extractCompanyId(body: Record<string, unknown>, token: string): string {
  ...
  // 2. ESTRUCTURA REAL: user.companyUsers[0].company.id
  const companyUsers = user?.companyUsers as Array<Record<string, unknown>> | undefined;
  if (Array.isArray(companyUsers) && companyUsers.length > 0) {
    const cu = companyUsers[0];
    const cuCompany = cu?.company as Record<string, unknown> | undefined;
    if (cuCompany?.id) return String(cuCompany.id);
    if (cu?.companyId) return String(cu.companyId);
  }
  ...
  const tries: Array<() => unknown> = [
    ...
    () => user?.companyId,
    () => user?.company_id,
    () => (user?.company as Record<string,unknown>)?.id,
    () => (user?.companies as Array<Record<string,unknown>>)?.[0]?.id,
    () => body.companyId,
    () => body.company_id,
  ];
...
function Spinner() { return <div style={{width:'15px',height:'15px',flexShrink:0,border:'2.5px solid rgba(255,255,255,0.3)',borderTopColor:'#fff',borderRadius:'50%',animation:'nxspin 0.7s linear infinite'}}/>; }
function ErrBox({msg}:Readonly<{msg:string}>) { return <div style={{background:'#FEF2F2',border:'1px solid #FECACA',color:'#DC2626',fontSize:'13px',borderRadius:'10px',padding:'10px 14px',marginBottom:'16px'}}>{msg}</div>; }
function OkBox({msg}:Readonly<{msg:string}>) { return <div style={{background:'#F0FDF4',border:'1px solid #BBF7D0',color:'#15803D',fontSize:'13px',borderRadius:'10px',padding:'10px 14px',marginBottom:'16px'}}>{msg}</div>; }
...
export default function LoginPage() {
  ...
  const [rPass,setRPass]=useState(''); const [rConf,setRConf]=useState('');
  ...
  const doLogin=useCallback(async(e:React.FormEvent)=>{
    ...
    try{
      ...
      const user=body.user as Record<string,unknown>|undefined;
      const userName=String(user?.fullName??user?.name??user?.email??body.name??lEmail);
      ...
      console.log('[Nexora Login] companyId guardado:',companyId||'⚠ VACÍO — revisar user.companyUsers');
  ...
  return(<>
    <style>{`@keyframes nxspin{to{transform:rotate(360deg)}}@keyframes nxfadeIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}.nx-card{animation:nxfadeIn 0.35s ease}.nx-btn:hover:not(:disabled){filter:brightness(1.08);transform:translateY(-1px)}.nx-btn:disabled{opacity:0.65;cursor:not-allowed}`}</style>
    <div style={{minHeight:'100vh',background:'linear-gradient(150deg,#EEF2FF 0%,#DBEAFE 55%,#E0F2FE 100%)',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px',fontFamily:'system-ui,-apple-system,sans-serif',overflow:'hidden'}}>
      <div className="nx-card" style={{width:'100%',maxWidth:'420px',background:'rgba(255,255,255,0.94)',backdropFilter:'blur(24px)',borderRadius:'24px',padding:'40px 40px 32px',boxShadow:'0 8px 48px rgba(0,0,0,0.10)',border:'1px solid rgba(255,255,255,0.75)'}}>
        ...
        {mode==='login'&&(<form onSubmit={doLogin} noValidate>
        ...
        {mode==='register'&&(<form onSubmit={doRegister} noValidate>
